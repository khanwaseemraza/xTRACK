...
const runAgentStep = async () => {
  ...
  useEffect(() => {
    ...
    if (showOverlay) drawOverlay(canvas, w, h, frame.objects);
  ...
  const clickToViewport = (ev: React.MouseEvent) => {
    const img = imgRef.current;
...
<pre className="small" style={{whiteSpace:"pre-wrap"}}>{agentLog}</pre>
      ...
      <div className="viewer">
        <div className="stage"
          onClick={(ev)=>{ const {x,y}=clickToViewport(ev); wsRef.current?.send({type:"click", x, y}); }}>
          {imgSrc ? (
